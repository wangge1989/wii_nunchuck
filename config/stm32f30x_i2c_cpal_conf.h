...
#ifndef __STM32F30X_I2C_CPAL_CONF_H
#define __STM32F30X_I2C_CPAL_CONF_H
...
------ After configuring CPAL firmware functionality , You should proceed by configuring hardware used with CPAL
       (please refer to stm32f30x_i2c_cpal_hal.h file).
...
------ Callbacks are routines that let you insert your own code in different stages of communication and for handling
       device errors. Their prototypes are declared by the CPAL library (if the relative define in this stm32f30x_i2c_cpal_conf.h is enabled)
       ...
       There are three types of Callbacks: Transfer User Callbacks, Error User Callbacks and Timeout User Callbacks:
        ...
        -a- Transfer User Callbacks :
          ...
          ** CPAL_I2C_SLAVE_WRITE_UserCallback(CPAL_InitTypeDef* pDevInitStruct)
               This function is called when master requests Read operation in Listen mode only.

          ** CPAL_I2C_SLAVE_READ_UserCallback(CPAL_InitTypeDef* pDevInitStruct)
               This function is called when master requests write operation in Listen mode only.
        ...
        -c- Timeout User Callbacks :
          ...
          ** CPAL_I2C_TIMEOUT_Manager()
               ...
               This function is already implemented in the CPAL drivers (stm32f30x_i2c_cpal.c file). It should be called periodically
     ...
     your application (their prototypes are declared in stm32f30x_i2c_cpal.h file).
     ...
     Example of using CPAL_I2C_BERR_UserCallback :
             ...
             -2- Comment define relative to CPAL_I2C_BERR_UserCallback in stm32f30x_i2c_cpal_conf.h file:
...
------ The driver API functions Prototypes are in stm32f30x_i2c_cpal.h file.
...
#define _CPAL_TIMEOUT_DEINIT()          ;       /*<! Disable the systick timer */
...
#endif /* __STM32F30X_I2C_CPAL_CONF_H */
